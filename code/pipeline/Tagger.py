...
class Tagger ():
    ...
    def get (self, df, field = 'tweet'):
        ...
        if self.language in ['hi', 'mr', 'ta']:
            processors = 'tokenize,pos'
        
        
        # @var nlp Stanza Pipeline
        nlp = stanza.Pipeline (lang = self.language, processors = processors)
        ...
        operation = df_dask[field].astype ('str').apply (lambda document: get_pos_and_ner (document) if document else ['', ''])
