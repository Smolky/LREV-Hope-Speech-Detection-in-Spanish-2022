...
class Dataset ():
    ...
    def assign_default_splits (self, df, label = 'label', field = '__split', stratify = True):
        ...
        This is done by generating a stratified sampling
        ...
        if stratify:
            ...
            # @var grouped_df DataFrame We first randomize the dataset, and 
            grouped_df = df.sample (frac = 1).groupby (label)
            
            
            items = [np.split (g, [int (self.get_train_size () * len (g)), int ((1 - self.get_test_size ()) * len (g))]) for i, g in grouped_df]
            
            train = pd.concat ([t[0] for t in items])
            val = pd.concat ([t[1] for t in items])
            test = pd.concat ([v[2] for v in items])
    ...
    def is_imbalanced (self, threshold = .15):
        """ 
    ...
    def preprocess (self, df, pipeline = [], field = 'tweet'):
        ...
        # @var preprocessing_steps List Basic stuff
        preprocessing_steps = [
            'remove_urls', 'remove_hashtags', 'remove_mentions', 'remove_whitespaces', 'strip'
        ]

        
        # First, solve specific language problems
        # For example, English contractions before removing punctuation
        # @todo possessive pronoum
        if self.get_dataset_language () == 'en':
            df[field] = preprocess.expand_acronyms (df[field], preprocess.english_contractions)


        # OR Spanish custom stuff
        if self.get_dataset_language () == 'es':
            df[field] = preprocess.remove_elongations (df[field])
            ...
            df[field] = preprocess.remove_digits (df[field], replace = '[NUMERO]')


        # Some languages can do it better
        if self.get_dataset_language () in ['es', 'en', 'fr', 'it']:
            preprocessing_steps = [
                'remove_urls', 'remove_hashtags', 'remove_mentions', 'remove_whitespaces', 
                'remove_emojis', 'remove_quotations', 'remove_punctuation', 
