"""
    ACL Homophobia output generation
...
def main ():
    ...
    dataset_name = 'aclhomophobia'
    
    
    # @var corpus_name String
    corpora = ['tamil', 'english', 'tamil-english']


    # var parser
    parser = DefaultParser (description = 'ACL Homophobia Submission')
    ...
    # @var tests Dict
    tests = {
        'run-01': {'folder': 'deep-learning-se'},
        'run-02': {'folder': 'deep-learning-lf-se-we'},
        'run-03': {'folder': 'deep-learning-all-features'}
    }
    ...
    for corpus_name in corpora:
        ...
        if corpus_name == 'tamil':
            file_path = 'umuteam_tamil.tsv'
        elif corpus_name == 'english':
            file_path = 'umuteam_en.tsv'
        else:
            file_path = 'umuteam_ta-en.tsv'
    ...
    with ZipFile (zip_file_path, 'w') as zipf:
        zipf.write (dataset.get_working_dir ('..', 'runs', args.test, 'umuteam_en.tsv'), arcname = 'umuteam_en.tsv')
        zipf.write (dataset.get_working_dir ('..', 'runs', args.test, 'umuteam_tamil.tsv'), arcname = 'umuteam_tamil.tsv')
        zipf.write (dataset.get_working_dir ('..', 'runs', args.test, 'umuteam_ta-en.tsv'), arcname = 'umuteam_ta-en.tsv')
